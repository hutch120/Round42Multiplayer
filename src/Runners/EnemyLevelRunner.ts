...
import {
  ...
  setPhaserLocations,
  setTotalEnemies
} from '../State/EnemyLevel/EnemyLevelActions'
import { EnemyState } from '../State/EnemyLevel/EnemyState'
import {
  enemeyHit as enemyHit,
  increaseScore,
  phaserFired,
  removePhaser,
  setPause
...
function draw(): void {
  ...
  phaserLocations.forEach((pf) =>
    renderFrame(pf.left, pf.top, [[CGAColors.yellow, CGAColors.yellow]])
  )
}
...
function handlePhaser(tick: number): void {
  ...
  if (
    ...
    // nozzle location can be undefined so we need a truthy check.
    ...
    if (randomEnemyCenter !== undefined) {
      ...
      const phaserLocations = getPhaserLocations(
        playerNozzleLocation.left,
        playerNozzleLocation.top,
        randomEnemyCenter.left,
        randomEnemyCenter.top
      )
