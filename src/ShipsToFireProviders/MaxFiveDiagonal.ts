...
export default function maxFiveDiagonal(tick: number): ShipToFire[] {
  ...
  for (let b = 0; b < bulletsToFire; b++) {
    ...
    const mayFire =
      !bullets.some((bullet) => bullet.owner === enemy.enemyId) || enemies.length < maxBullets
...
function getBestCandiates(tick: number): Candidates {
  ...
  for (const enemy of enemies) {
    ...
    if (center) {
      const angleToPlayer = calculateAngle(
        center.left,
        center.top,
        playerState.left,
        playerState.top
  ...
  const sortFunction =
    above > below ? (a: number, b: number) => a < b : (a: number, b: number) => a > b
