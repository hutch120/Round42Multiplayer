...
export function GameOptions(): JSX.Element {
  ...
  const [currentKeyBind, setCurrentKeyBind] = useState<keyof KeybindingsState | undefined>(
    undefined
  )

  const settings = useSelector<ApplicationState, SettingsState>((state) => state.settingsState)
  const gameInProgress = useSelector<ApplicationState, boolean>(
    (state) => state.gameState.gameInProgress
  )

  const { gameSpeed, keybindings, playSound } = settings
          ...
          {!gameInProgress && (
            <span style={Styles.textStyle}>
              <AsciiSlider
                ...
                disabled={listening}
              />{' '}
              <span style={{ display: 'inline-block', textAlign: 'right', width: '2em' }}>
                {gameSpeed}%
              </span>
            </span>
          )}
...
function cell(
  description: string,
  currentBind: string,
  keybindingKeyword: keyof KeybindingsState,
  changeBinding: (key: keyof KeybindingsState) => void,
  listening: boolean,
  currentKeyBind: keyof KeybindingsState | undefined
): ReactElement {
  return (
    <tr>
      ...
      <td style={Styles.tableTextCellStyle}>
        {listening && currentKeyBind === keybindingKeyword ? (
          ...
          <HoverButton
            disabled={listening}
            onClick={() => changeBinding(keybindingKeyword)}
            text="Edit"
          />
        )}
      </td>
