...
export function getBulletParticleState(
  ...
  const particle = getParticleState(
    left,
    top,
    speed,
    angle,
 ...
 */
export function explosionShrapnellProvider(
  left: number,
  top: number,
  explosion: Explosion
): ParticleState[] {
  ...
  for (let i = 0; i < explosion.particleFrameIndexes.length; i++) {
    ...
    const speed = calculateSpeed(
      explosion.useSpeed ? explosion.speed : explosion.speeds[i],
      appState().speedState.gameSpeed
    )

    const p = getParticleState(left, top, speed, angle, particleFrame, explosion.acceleration, 0, 0)
    particles.push(p)
  }
