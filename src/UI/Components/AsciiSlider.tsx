...
export function AsciiSlider({
  ...
  onChange,
  disabled
}: AsciiSliderProperties): JSX.Element {
...
function getAsciiSliderString(
  ...
  const transChar =
    transFactor != 0 && transFactor != 1
      ? chars[charsOffset + Math.floor(transFactor * (chars.length - charsOffset))]
      : ''
  return `${charL.repeat(filledCount)}${transChar}${char0.repeat(
    lineCount - filledCount - transChar.length
  )}`
}
