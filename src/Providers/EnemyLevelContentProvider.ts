...
const { gameField } = dimensionProvider()

export default function enemyLevelContentFactory(enemy: Enemies): {
  bulletRunner?: BulletRunner
  enemies: BaseEnemy[]
} {
  switch (enemy) {
    case 'bird':
    case 'bird-fire': {
      ...
      if (enemy === 'bird-fire') {
        bulletRunner = new BulletRunner(
          CGAColors.brown,
          () => appState().speedState.bullets.bird,
          maxFiveDiagonal
        )
      }
    ...
    case 'robots-random': {
      ...
      if (enemy === 'robot') {
        bulletRunner = new BulletRunner(
          CGAColors.lightRed,
          () => appState().speedState.bullets.robot,
          firstEnemyOccasionalDown
        )
        ...
        enemies = getRandomSpawnLocations(
          14,
          gameField.top,
          Locations.robot.scatteredMaxBottom
        ).map((location) => enemyFactory(enemy, location))
      } else {
    ...
    case 'orb':
    case 'orb-up-down': {
      const enemies = orbSpawnLocations.map((location, index) =>
        enemyFactory(enemy, location, index)
      )
      const bulletRunner = new BulletRunner(
        CGAColors.magenta,
        () => appState().speedState.bullets.orb,
        maxFiveDiagonal
      )
    ...
    case 'spinner': {
      ...
      const bulletRunner = new BulletRunner(
        CGAColors.white,
        () => appState().speedState.bullets.spinner,
        maxFiveDiagonal
      )
    ...
    case 'balloon': {
      ...
      const bulletRunner = new BulletRunner(
        CGAColors.blue,
        () => appState().speedState.bullets.balloon,
        maxFiveDiagonal
      )
    ...
    case 'piston': {
      const enemies = elevenInALine(Locations.Piston.topStart).map((location) =>
        enemyFactory(enemy, location)
      )
      const bulletRunner = new BulletRunner(
        CGAColors.blue,
        () => appState().speedState.bullets.balloon,
    ...
    case 'diabolo-hard': {
      ...
      const bulletRunner = new BulletRunner(
        CGAColors.yellow,
        () => appState().speedState.bullets.diabolo,
        maxFiveDiagonal
      )
    ...
    case 'devil': {
      ...
      const bulletRunner = new BulletRunner(
        ...
        () => appState().speedState.bullets.devil,
        (tick) => fireDownAimed(tick, 3)
      )
    ...
    case 'crab': {
      const enemies = elevenInALine(Locations.Crab.topStart).map((location) =>
        enemyFactory(enemy, location)
      )
      const bulletRunner = new BulletRunner(
        CGAColors.lightRed,
        () => appState().speedState.bullets.crab,
        maxFiveDiagonal
      )
    ...
    case 'bat': {
      ...
      const bulletRunner = new BulletRunner(
        CGAColors.yellow,
        () => appState().speedState.bullets.bat,
        maxFiveDiagonal
      )
    ...
    case 'cloaking-orb': {
      const enemies = getRandomSpawnLocations(
        15,
        gameField.top,
        Locations.CloakingOrb.maxBottom
      ).map((location) => enemyFactory(enemy, location))
      ...
      const bulletRunner = new BulletRunner(
        CGAColors.lightRed,
        () => appState().speedState.bullets.cloakingOrb,
        (tick) => fireDownAimed(tick, 5, [0, 1, 3])
      )
    ...
    case 'fish': {
      ...
      const bulletRunner = new BulletRunner(
        CGAColors.lightGreen,
        () => appState().speedState.bullets.fish,
        (tick) => fireDownAimed(tick, 5, [0, 1])
      )
