...
const { pixelSize, fullGameWidth, gameField } = dimensionProvider()
...
export default class WarpLevel implements ILevel {
    ...
    return new Promise((resolve) => {
      ...
      dispatch(
        setPlayerLocationData(
          Locations.Player.spawnLocation.left,
          Locations.Player.spawnLocation.top
        )
      )
      ...
      drawLevelBanner(gameState.level, () => {
        ...
        const warpGate = this.calculateWarpGate(
          gameField.left,
          gameField.right,
          gameState.warpLevelSteps.stepsX,
          gameState.warpLevelSteps.stepsY
        )
        ...
        this.gameLoopSubscriptions.push(
          GameLoop.registerBackgroundDrawing(() => drawWarpBackground(additionalColor, warpGate))
        )

        // Add a function to the GameLoop that will check if a level has been won.
        this.gameLoopSubscriptions.push(
          GameLoop.registerLevelWonMonitor(() => this.monitorLevelWon())
        )
        ...
        this.gameLoopSubscriptions.push(
          GameLoop.registerUpdateState((tick) => this.hitDetection(tick, badSpace))
        )
   ...
   */
  private getWallHitboxes(
    warpGate: GameRectangle[]
  ): Array<{ left: GameRectangle; right: GameRectangle }> {
    return warpGate.map((wg) => {
      return {
        left: {
          left: gameField.left,
          right: wg.left,
  ...
  private hitDetection(
    tick: number,
    badSpace: Array<{ left: GameRectangle; right: GameRectangle }>
  ): void {
