...
const { pixelSize2x } = dimensionProvider()
 ...
 */
export default function fireDownAimed(
  tick: number,
  maxBullets: number,
  fireOnFrames?: number[]
): ShipToFire[] {
  ...
  if (fireOnFrames !== undefined) {
    candidates = candidates.filter(
      (es) => es.currentFrameIndex !== undefined && fireOnFrames.indexOf(es.currentFrameIndex) > -1
    )
  }
  ...
  if (enemyToFire !== undefined) {
    const queuedTofire =
      returnValue.find((e) => e.enemy.enemyId === enemyToFire.enemyId) !== undefined
    const mayFire =
      enemies.length < maxBullets || !bullets.some((bullet) => bullet.owner === enemyToFire.enemyId)
...
function getCandidates(tick: number): EnemyState[] {
  ...
  const {
    hitboxes: { bottom: bottomhitbox }
  } = playerState

  return getShipsReadyToFire(tick).filter(
    (enemy) =>
      enemy.hitbox.left + pixelSize2x >= bottomhitbox.left &&
      enemy.hitbox.right <= bottomhitbox.right + pixelSize2x
  )
}
