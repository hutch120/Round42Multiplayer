...
const { width: playerWidth, height: playerHeight } = getFrameDimensions(getPlayerFrame())
...
export function playerMovementHandler(speedX: number, speedY: number): void {
  ...
  const { left: nextX, top: nextY } = getNextLocationForPlayer(
    angle,
    speedX,
    speedY,
    playerState.left,
 ...
 */
function getNextLocationForPlayer(
  angle: number,
  speedX: number,
  speedY: number,
  currentX: number,
