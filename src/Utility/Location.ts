 ...
 */
export function fallsWithinGameField(
  left: number,
  right: number,
  top: number,
  bottom: number
): boolean {
  const res = fallsWithin(
    left,
    right,
    top,
    bottom,
 ...
 */
export function getNextLocationAndAngle(
  currentLeft: number,
  currentTop: number,
  angle: number,
  speed: number,
  ...
  topLimit: number,
  bottomLimit: number
): { location: GameLocation; angle: number } {
