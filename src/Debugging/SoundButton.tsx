...
export default function SoundButton(props: {
  text: string
  src: string
  onPlay: (src: string, begin: number, end: number) => void
  sprite?: number[]
}): JSX.Element {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row' }}>
      <button style={{ width: '150px' }} onClick={handleClick}>
        {props.text}
      </button>
      <input
        style={{ width: '30px', backgroundColor: customBegin ? 'yellow' : 'white' }}
        value={begin}
        onChange={(e) => setBegin(e.target.value)}
      ></input>
      <input
        style={{ width: '30px', backgroundColor: customEnd ? 'yellow' : 'white' }}
        value={end}
