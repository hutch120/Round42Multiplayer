...
const { pixelSize, gameField } = dimensionProvider()
...
export default function enemyFactory(
  ...
  switch (enemy) {
    ...
    case 'robots-random': {
      ...
      const frameProvider = new BackAndForthFrameProvider(
        enemy === 'robot' ? 0 : getRandomFrameKeyIndex(frames)
      )
      const locationProvider: ILocationProvider = new SideAppearOtherSide(
        location.left,
        location.top,
      ...
      return new DefaultEnemy(
        ...
        frameProvider,
        {
          explosionColor: robotColor,
          explosionParticleColor: robotColor,
          varyingEnemyColor: robotColor
        }
      )
    }
    ...
    case 'balloon': {
      ...
      const locationProvider = new Wobble(
        location.left,
        location.top,
        () => appState().speedState.movement.balloon,
        0,
        width,
        height,
        200
    ...
    case 'bat': {
      ...
      return new DefaultEnemy(
        ...
        frameProvider,
        {
          explosionColor: CGAColors.lightGray,
          explosionParticleColor: CGAColors.lightGray,
          varyingEnemyColor: color
        }
      )
    }
    ...
    case 'cloaking-orb': {
      ...
      const locationProvider = new CloakingOrbLocationProvider(
        location.left,
        location.top,
        frames.length - 2,
        frameProvider
