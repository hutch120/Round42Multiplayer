...
export default class BirdEnemy extends BaseEnemy {
  ...
  constructor(
    ...
    super(frameChangeTime, getOffsetFrames, getExplosion, locationProvider, frameProvider)

    this.colorTickHandler = new TickHandler(40, () => this.onColorChange())

    Mutators.FrameSetColor(this.explosion.explosionCenterFrame, CGAColors.white)
    this.explosion.particleFrames.forEach((pf) => Mutators.FrameSetColor(pf, CGAColors.white))
  }
  ...
  private setCurrentFrameColor(frame: Frame) {
    Mutators.FrameSetColor(frame, getRandomArrayElement(ColorSchemes.birds))
